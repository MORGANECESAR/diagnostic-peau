...
const C = {
  ink: "#26211A",         // noir chaud : textes et titres. Remplace le vert profond de l'ancienne charte.
  inkSoft: "#3D3529",     // brun sombre : tous les aplats (boutons). Le noir chaud reste réservé au texte.
...
const SOIN_MAP = {
  glow: {
    ...
    produits: { coreen: "Beauty of Joseon Glow Serum : Propolis + Niacinamide", parapharmacie: "Vichy Liftactiv Vitamin C Sérum Éclat", bio: "Aroma-Zone Sérum Vitamine C 10%", budget: "The Ordinary Vitamin C Suspension 23% + HA 2%", institut: "Genosys MVS – Multi Vita Radiance Serum", institutBio: "Bio by Oxalia Sérum Éclat Absolu + Crème Éclat Absolu SPF30" },
  },
  renaissance: {
  ...
  reparation: {
    ...
    produits: { coreen: "SKIN1004 Centella Probio-Cica Enrich Cream", parapharmacie: "La Roche-Posay Cicaplast Baume B5", bio: "Aroma-Zone Beurre de Karité Brut BIO", budget: "CeraVe Crème Hydratante Visage", institut: "Genosys HSC – Hydro Soothing Cream", institutBio: "Bio by Oxalia Crème Tentation Fondante + Sérum Tentation" },
  },
  // Densité et Renaissance répondent aux mêmes préoccupations par deux voies différentes :
  // Renaissance est le protocole coréen, Densité le protocole bio correctif.
  // Le départage se fait sur le type de peau (voir computeRecommendedSoins).
  densite: {
    name: "Parenthèse Densité",
    tagline: "Densité • Nutrition • Confort",
    concerns: ["rides", "relachement", "ovale_visage", "cou_decollete"],
    produits: { coreen: "Missha Time Revolution Night Repair Ampoule", parapharmacie: "Nuxe Merveillance Lift Crème Fermeté", bio: "Aroma-Zone Sérum Concentré Collagène Végétal 1,5%", budget: "The Inkey List Bakuchiol", institut: "Dermeden Sérum Intense Nuit (rétinol encapsulé, peptides matrikines)", institutBio: "Bio by Oxalia Expert Pro-Âge – Sérum et Soin de Jour Pro-Âge" },
  },
};
...
const OTHER_SOINS = [
  {
    ...
    produits: {
      ...
      institutBio: "Bio by Oxalia Riz aux Amandes (gommage) + Masque visage en coton bio (masque)",
    },
  },
  {
...
// puis Renaissance (le plus complet), puis Densité, puis Glow. Modifiable : c'est un choix de praticienne.
const SOIN_PRIORITY = ["reparation", "renaissance", "densite", "glow"];

// Sur une peau déclarée mature, le même score bascule vers la Parenthèse Densité :
// c'est le protocole bio correctif, pensé pour les peaux âgées.
const SOIN_PRIORITY_MATURE = ["reparation", "densite", "renaissance", "glow"];

function computeRecommendedSoins(selectedConcernIds, selectedSkinTypeIds = []) {
  ...
  const ordre = selectedSkinTypeIds.includes("mature") ? SOIN_PRIORITY_MATURE : SOIN_PRIORITY;
  const top = scores.filter((s) => s.score === topScore);
  top.sort((x, y) => ordre.indexOf(x.key) - ordre.indexOf(y.key));
  return [top[0].soin];
}
...
const CONCERNS = [
  ...
  { id: "terne", label: "Teint terne, manque d'éclat", actifs: ["Vitamine C", "Niacinamide", "Exfoliation douce (PHA)"],
    produits: { coreen: "Beauty of Joseon Glow Serum : Propolis + Niacinamide", parapharmacie: "Vichy Liftactiv Vitamin C Sérum Éclat", bio: "Aroma-Zone Sérum Vitamine C 10%", budget: "The Ordinary Vitamin C Suspension 23% + HA 2%", institut: "Genosys MVS – Multi Vita Radiance Serum", institutBio: "Bio by Oxalia Sérum Éclat Absolu (concentré éclat & anti-taches)" } },
  { id: "rides", label: "Rides et ridules", actifs: ["Peptides", "Rétinol ou bakuchiol", "PDRN"],
    produits: { coreen: "Missha Time Revolution Night Repair Ampoule", parapharmacie: "La Roche-Posay Redermic R Rétinol Pur", bio: "Aroma-Zone Sérum Concentré Collagène Végétal 1,5%", budget: "The Inkey List Bakuchiol", institut: "Dermeden Sérum Intense Nuit (rétinol encapsulé, peptides matrikines)", institutBio: "Bio by Oxalia Gamme Expert Pro-Âge – Masque Pro-Âge Combleur de Rides" } },
  { id: "relachement", label: "Relâchement, perte de fermeté", actifs: ["Collagène", "Élastine", "Peptides fermeté", "Massage liftant"],
    produits: { coreen: "Beauty of Joseon Revive Serum : Ginseng + Snail Mucin", parapharmacie: "Vichy Liftactiv Collagen Specialist", bio: "Aroma-Zone Crème Riche Collagène & Spilanthes", budget: "The Ordinary Buffet (complexe peptides)", institut: "Genosys MFC – Multi Functional Anti Wrinkle Cream", institutBio: "Bio by Oxalia L'Onctueux – Masque Visage Anti-Âge Régénérant" } },
  { id: "taches", label: "Taches pigmentaires", actifs: ["Vitamine C", "Arbutine", "Niacinamide", "SPF quotidien"],
    produits: { coreen: "Some By Mi Galactomyces Pure Vitamin C Glow Serum", parapharmacie: "La Roche-Posay Pigmentclar Sérum", bio: "Aroma-Zone Sérum Vitamine C 10%", budget: "The Ordinary Alpha Arbutin 2% + HA", institut: "Genosys SWS – Skin Whitening Serum", institutBio: "Bio by Oxalia Sérum Éclat Absolu (concentré éclat & anti-taches)" } },
  { id: "imperfections", label: "Imperfections, points noirs", actifs: ["Niacinamide", "Acide salicylique (BHA)", "Argile"],
    produits: { coreen: "COSRX BHA Blackhead Power Liquid", parapharmacie: "La Roche-Posay Effaclar Duo(+)", bio: "Aroma-Zone Sérum Niacinamide 10%, Cuivre & Zinc", budget: "CeraVe Gel Moussant SA Renewing", institut: "Genosys PCC – Problem Control Cream", institutBio: "Bio by Oxalia Crème Fluide Épure (soin matifiant)" } },
  { id: "rougeurs", label: "Rougeurs, sensibilité, couperose", actifs: ["Centella asiatica", "Panthénol", "Actifs apaisants sans parfum"],
    produits: { coreen: "Anua Heartleaf 77% Soothing Toner", parapharmacie: "Avène Antirougeurs Jour", bio: "Aroma-Zone Gel d'Aloe Vera Bio", budget: "Cetaphil Crème Hydratante Apaisante", institut: "Genosys HSC – Hydro Soothing Cream", institutBio: "Bio by Oxalia Brume de Douceur (lotion apaisante)" } },
  { id: "pores", label: "Pores dilatés, grain irrégulier", actifs: ["Niacinamide", "Acide salicylique", "Argile"],
    produits: { coreen: "Some By Mi AHA BHA PHA 30 Days Miracle Serum", parapharmacie: "Vichy Normaderm Sérum", bio: "Aroma-Zone Argile Verte Surfine", budget: "The Ordinary Niacinamide 10% + Zinc 1%", institut: "Genosys Epi Turnover Boosting Peeling Gel", institutBio: "Bio by Oxalia Crème Fluide Épure (soin matifiant)" } },
  { id: "cernes", label: "Cernes, poches", actifs: ["Caféine", "Vitamine K", "Peptides contour des yeux"],
    produits: { coreen: "AHC Ultimate Real Eye Cream for Face", parapharmacie: "Avène Physiolift Yeux", bio: "Aroma-Zone Roll-on Contour des Yeux à la Caféine", budget: "The Ordinary Caffeine Solution 5% + EGCG", institut: "Genosys Eye Cell – Eye Contour Serum", institutBio: "Bio by Oxalia Gamme Expert Pro-Âge – Contour des Yeux" } },
  { id: "cicatrices", label: "Cicatrices (acné, autres)", actifs: ["Centella asiatica", "Vitamine C", "Actifs cicatrisants"],
    produits: { coreen: "SKIN1004 Centella Probio-Cica Enrich Cream", parapharmacie: "La Roche-Posay Cicaplast Baume B5", bio: "Aroma-Zone Huile de Rose Musquée du Chili Bio", budget: "CeraVe Baume Réparateur", institut: "Dermeden Cicaderm – Soin Réparateur Cicatrisant Arnica+", institutBio: "Bio by Oxalia Crème Douceur Végétale (soin nourrissant intense)" } },
  ...
  { id: "acne_hormonale", label: "Acné hormonale adulte (menton, mâchoire, cycle)", actifs: ["Acide azélaïque", "Niacinamide", "Zinc", "Acide salicylique"],
    produits: { coreen: "Anua Azelaic Acid 10 Hyaluron Redness Soothing Serum", parapharmacie: "La Roche-Posay Effaclar Duo(+)", bio: "Aroma-Zone Sérum Concentré Acide Azélaïque 10%", budget: "The Ordinary Azelaic Acid Suspension 10%", institut: "Genosys PCC – Problem Control Cream", institutBio: "Bio by Oxalia Crème Fluide Épure (soin matifiant)" } },
  { id: "ovale_visage", label: "Perte de définition de l'ovale du visage", actifs: ["Peptides fermeté", "Collagène", "DMAE", "Massage liftant"],
    produits: { coreen: "Missha Time Revolution Night Repair Ampoule", parapharmacie: "Vichy Liftactiv Collagen Specialist", bio: "Aroma-Zone Crème Riche Collagène & Spilanthes", budget: "The Ordinary Buffet (complexe peptides)", institut: "Genosys MFC – Multi Functional Anti Wrinkle Cream", institutBio: "Bio by Oxalia L'Onctueux – Masque Visage Anti-Âge Régénérant (effet tenseur, redessine les contours)" } },
  { id: "exces_sebum", label: "Excès de sébum, brillance", actifs: ["Niacinamide", "Argile", "Zinc", "Acide salicylique"],
    produits: { coreen: "Some By Mi AHA BHA PHA 30 Days Miracle Serum", parapharmacie: "Vichy Normaderm Sérum", bio: "Aroma-Zone Argile Verte Surfine", budget: "The Ordinary Niacinamide 10% + Zinc 1%", institut: "Dermeden Hydra Protocole – Crème Légère Matifiante", institutBio: "Bio by Oxalia Crème Fluide Épure (soin matifiant)" } },
];
  ...
  return (
    <div className="flex items-center justify-between mt-10 print:hidden">
      <button
        ...
        style={{ color: C.ink, opacity: onBack ? 1 : 0, pointerEvents: onBack ? "auto" : "none" }}
      >
        ← {backLabel}
      </button>
      <button
        ...
        style={{ backgroundColor: nextDisabled ? "#C9BC9E" : C.inkSoft, opacity: nextDisabled ? 0.6 : 1 }}
      >
        {nextLabel} →
      </button>
    </div>
...
h1{font-size:22px;color:#26211A;margin-bottom:2px;}
...
.label{text-transform:uppercase;font-size:12px;letter-spacing:0.08em;font-weight:600;color:#26211A;margin:0 0 8px;font-family:Arial,sans-serif;}
p{font-family:Arial,sans-serif;}
footer{margin-top:28px;background:#3D3529;color:#FBF6EC;padding:14px 18px;border-radius:8px;font-size:12px;text-align:center;font-family:Arial,sans-serif;}
</style></head><body>
    ...
    >
      <div className="w-full max-w-2xl">
        ...
        <div className="flex items-center justify-between mb-8 print:hidden">
          <div>
            <p style={{ fontFamily: "Georgia, serif", color: C.ink }} className="text-lg leading-tight">
              L'Institut <span style={{ color: C.orange, fontStyle: "italic" }}>Morgane César</span>
            </p>
            <p className="tracking-widest" style={{ color: C.ink, letterSpacing: "0.15em", fontSize: "11px" }}>
        ...
        >
          ...
          {step === 0 && (
            <div className="text-center py-6">
              ...
              <h1 style={{ fontFamily: "Georgia, serif", color: C.ink }} className="text-3xl sm:text-4xl mb-4">
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-sm font-semibold mt-6 mb-2" style={{ color: C.ink }}>Peau masculine ?</p>
              <div className="flex flex-wrap gap-2">
                {["Oui", "Non"].map((v) => (
                  <Choice key={v} selected={a.peauMasculine === v} onClick={() => set("peauMasculine", v)}>{v}</Choice>
                ))}
              </div>
              <p className="text-sm font-semibold mt-6 mb-2" style={{ color: C.ink }}>Tranche d'âge</p>
              <div className="flex flex-wrap gap-2">
                {AGE_RANGES.map((v) => (
          ...
          {step === 4 && (
            <div>
              ...
              <div className="space-y-5">
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Exposition au soleil</p>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Protection solaire quotidienne</p>
                  <div className="flex flex-wrap gap-2">
                    {["Oui", "Non"].map((v) => (
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Tabac</p>
                  <div className="flex flex-wrap gap-2">
                    {["Oui", "Non"].map((v) => (
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Sommeil</p>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Niveau de stress perçu</p>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Alimentation</p>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Contraception hormonale (pilule, stérilet hormonal...)</p>
                  <div className="flex flex-wrap gap-2">
          ...
          {step === 5 && (
            <div>
              ...
              <div className="space-y-5">
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Quelles étapes utilisez-vous déjà ?</p>
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Marques que vous utilisez actuellement (facultatif)</p>
                  <input
                    type="text"
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Fréquence du nettoyage du visage</p>
                  <div className="flex flex-wrap gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Utilisez-vous déjà du rétinol ou un exfoliant (AHA/BHA) ?</p>
                  <div className="flex flex-wrap gap-2">
                    {["Oui", "Non"].map((v) => (
                ...
                <div>
                  <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Un actif ou produit a déjà provoqué une réaction ? (facultatif)</p>
                  <input
                    type="text"
          ...
          {step === 6 && (
            <div>
              ...
              <div className="mt-4">
                <p className="text-sm font-semibold mb-2" style={{ color: C.ink }}>Allergies cutanées connues (facultatif)</p>
                <input
                  type="text"
          ...
          {step === 8 && (
            <div>
              ...
              <h2 style={{ fontFamily: "Georgia, serif", color: C.ink }} className="text-2xl sm:text-3xl text-center mb-1">
              ...
              <div className="rounded-lg p-5 mb-5" style={{ backgroundColor: C.cream }}>
                <p className="text-xs uppercase tracking-wide font-semibold mb-1" style={{ color: C.ink }}>Profil de peau</p>
              ...
              {(a.routineEtapes.length > 0 || a.marquesActuelles) && (
                <div className="rounded-lg p-5 mb-5" style={{ backgroundColor: "white", border: "1px solid #EFE6D2" }}>
                  <p className="text-xs uppercase tracking-wide font-semibold mb-1" style={{ color: C.ink }}>Routine actuelle</p>
              ...
              {selectedConcerns.length > 0 && (
                <div className="mb-5">
                  <div className="flex flex-wrap items-baseline justify-between gap-2 mb-3">
                    <p className="text-xs uppercase tracking-wide font-semibold" style={{ color: C.ink }}>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {c.actifs.map((act) => (
                            <span
                              ...
                              style={{ backgroundColor: C.ivory, color: C.ink, border: `1px solid ${C.goldLight}55` }}
                            >
                              {act}
                            </span>
                          ))}
              ...
              {a.objectifs && (
                <div className="mb-5">
                  <p className="text-xs uppercase tracking-wide font-semibold mb-2" style={{ color: C.ink }}>Attentes exprimées</p>
                ...
                <div className="rounded-lg p-5 mb-5" style={{ backgroundColor: C.cream, border: "1px solid #DEC89A" }}>
                  ...
                  {recommendedSoins.map((s) => (
                    <div key={s.name} className="mb-4 last:mb-0">
                      <p style={{ fontFamily: "Georgia, serif", color: C.ink }} className="text-lg">{s.name}</p>
                ...
                {sent ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      style={{ backgroundColor: !consent || !a.email ? "#C9BC9E" : C.inkSoft }}
                    >
              ...
              <div className="mt-3 print:hidden">
                <button
                  ...
                  style={{ borderColor: C.inkSoft, color: C.inkSoft }}
                >
                  Nouveau diagnostic
                </button>
              </div>
